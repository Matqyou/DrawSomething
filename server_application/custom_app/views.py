...
from .rules import valid_username, valid_password, valid_email
from custom_app.models import User, Game, Recording, ColorBundle
...
def create_game(request):
    ...
    try:
        random_user: User = User.objects.exclude(user_id=user_id).order_by('?').first()
        if random_user is None:
            return JsonResponse({"code": "game_create_fail", "message": f"No opponents could be found at this time"},
                                status=404)

        party_ids = [user, random_user]
        random.shuffle(party_ids)
        party_a, party_b = party_ids
        ...
        return JsonResponse({
            ...
            "game": game.display_mini(base_url)
        }, status=200)
    except Exception as e:
        return JsonResponse({"error": str(e)}, status=400)
...
@allowed_methods(['POST'])
@authed_user
# Route for requestion the deletion of a game
def delete_game(request):
    ...
    try:
        ...
        # Successfully delete the requested game
        game.delete()
        return JsonResponse(
            {"code": "game_delete_success", "message": f"Game #{game_id} has been deleted", "game_id": game_id},
            status=200)
    except Exception as e:
        return JsonResponse({"error": str(e)}, status=400)
